...
namespace Persistence.RepositoryAdapter
{
    public class PostRepossitory : RepositoryBase<Post, Guid>, IPostRepository
    {
        ...
        public async Task<List<Post>> GetTopActivePostsWithNonExpiredServices(int top)
        {
            ...
            var result = await _dbContext.Posts
                ...
                .Where(p => p.PostServices.Any(ps => ps.Service. > now))
                .OrderBy(p => p.CreatedAt) // Optional: Sort by creation date or any other criteria
                .Take(top)
                .ToListAsync();
